...
describe('integração NF-e — import por XML', () => {
  ...
  it('importa a NF-e do XML com todos os campos e itens', async () => {
    const res = await api().post('/api/nfs/importar').set('Authorization', h()).send({ xml: xmlExemplo() });
  ...
  it('reimportar a mesma NF-e → 409', async () => {
    ...
    const dup = await api().post('/api/nfs/importar').set('Authorization', h()).send({ xml: xmlExemplo() });
    expect(dup.status).toBe(409);
  });

  it('XML que não é NF-e → 400', async () => {
    const res = await api().post('/api/nfs/importar').set('Authorization', h()).send({ xml: '<foo>x</foo>' });
  ...
  it('import exige gestor (motorista → 403)', async () => {
    ...
    const res = await api().post('/api/nfs/importar').set('Authorization', bearer(appToken)).send({ xml: xmlExemplo() });
